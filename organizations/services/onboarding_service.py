...
class OnboardingService:
    ...
    def request_documents(
        ...
        if applicant_notes:
            onboarding.applicant_notes = (
                onboarding.applicant_notes + f'\n\n{applicant_notes}'
            ).strip()
    ...
    def upload_document(
        ...
        fulfils_request_id: str | None = None,
    ) -> OnboardingDocument:
        if not onboarding.is_editable_by_applicant:
        ...
        if fulfils_request_id:
            DocumentRequest.objects.filter(
                id=fulfils_request_id, onboarding=onboarding, fulfilled_at__isnull=True
            ).update(fulfilled_by_document=doc, fulfilled_at=timezone.now())
        else:
            DocumentRequest.objects.filter(
                onboarding=onboarding, document_type=document_type, fulfilled_at__isnull=True
            ).update(fulfilled_by_document=doc, fulfilled_at=timezone.now())
