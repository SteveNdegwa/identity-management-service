...
def _get_category(category_id: str) -> PermissionCategory | None:
    try:
...
def _get_permission(permission_id: str) -> Permission | None:
    try:
...
def _get_system_user(system_user_id: str) -> SystemUser | None:
    try:
...
def _get_override(override_id: str) -> UserPermissionOverride | None:
    try:
...
def _parse_datetime(value):
    ...
    try:
        ...
        raise PermissionServiceError('expires_at must be a valid ISO datetime.') from exc
...
def permission_list_view(request: ExtendedRequest, system_id: str) -> JsonResponse:
    try:
        ...
        qs = (
            Permission.objects.filter(system=system).select_related('category').order_by('codename')
        )
...
def role_list_view(request: ExtendedRequest, system_id: str) -> JsonResponse:
    try:
        ...
        qs = (
            Role.objects.filter(system=system)
            .select_related(
                'country',
                'parent_role',
                'created_by_org',
        ...
        return ResponseProvider.success(roles=[_role_payload(role) for role in qs])
    except Exception as e:
    ...
    try:
        ...
        if parent_role_id := data.get('parent_role_id'):
            ...
            if not parent_role or parent_role.system_id != system.id:
                return ResponseProvider.bad_request(
                    error='invalid_parent_role', message='Parent role not found.'
                )
...
def override_list_view(request: ExtendedRequest, system_user_id: str) -> JsonResponse:
    try:
        ...
        qs = system_user.permission_overrides.select_related('permission', 'granted_by').order_by(
            '-created_at'
        )
        if is_active := request.GET.get('is_active'):
            qs = qs.filter(is_active=is_active.lower() == 'true')

        return ResponseProvider.success(overrides=[_override_payload(override) for override in qs])
    except Exception as e:
...
def override_create_view(request: ExtendedRequest, system_user_id: str) -> JsonResponse:
    ...
    try:
        ...
        if not permission:
            return ResponseProvider.bad_request(
                error='invalid_permission', message='Permission not found.'
            )
