...
def _get_client(data: dict) -> SystemClient | None:
    ...
    if not client_id:
        return None
    try:
        return SystemClient.objects.select_related('system').get(
...
def _system_user_payload(system_user: SystemUser) -> dict:
    ...
    return {
        ...
        'country_code': system_user.country.code if system_user.country else None,
        'country_code3': system_user.country.code3 if system_user.country else None,
...
def _post_registration_response(
    ...
    try:
        payload['contexts'] = [
            vars(context) for context in sso_service.get_ready_contexts(session, client)
        ]
...
def register_link_view(request: ExtendedRequest) -> JsonResponse:
    try:
        ...
        except (User.DoesNotExist, KeyError) as _exc:
...
def me_view(request: ExtendedRequest) -> JsonResponse:
    try:
        ...
        memberships = SystemUser.objects.filter(user=user, status='active').select_related(
            'system', 'organization', 'country', 'role'
        )
        return ResponseProvider.success(
...
def me_update_view(request: ExtendedRequest) -> JsonResponse:
    try:
        updated = account_service.update_profile(system_user=request.system_user, **request.data)
        return ResponseProvider.success(
            user=_user_payload(updated.user), system_user=_system_user_payload(updated)
        )
    except SelfRegistrationError as e:
...
def my_referrals_view(request: ExtendedRequest) -> JsonResponse:
    try:
        referrals = Referral.objects.filter(referrer=request.system_user).select_related(
            'system', 'referrer', 'referred'
        )
        return ResponseProvider.success(
...
def attach_my_referral_view(request: ExtendedRequest) -> JsonResponse:
    try:
        referral = referral_service.attach_referral(
            referred=request.system_user, referral_code=request.data.get('referral_code', '')
        )
        referral = Referral.objects.select_related('system', 'referrer', 'referred').get(
            id=referral.id
        )
...
def referral_list_view(request: ExtendedRequest) -> JsonResponse:
    try:
        ...
        qs = (
            Referral.objects.filter(system_id=system_id)
...
def identifier_list_view(request: ExtendedRequest) -> JsonResponse:
    ...
    return ResponseProvider.success(
        identifiers=[
            _contact_payload(user, IdentifierType.EMAIL),
            _contact_payload(user, IdentifierType.PHONE),
        ]
    )
...
def identifier_verify_initiate_view(request: ExtendedRequest, identifier_id: str) -> JsonResponse:
    try:
        if identifier_id not in (IdentifierType.EMAIL, IdentifierType.PHONE):
            return ResponseProvider.bad_request(
                error='invalid_identifier', message="Identifier must be 'email' or 'phone'."
            )
        ...
        value = (
            request.sso_session.user.email
            if identifier_id == IdentifierType.EMAIL
            else request.sso_session.user.phone_number
        )
        return ResponseProvider.success(
            verification_id=str(verification.id),
            identifier_type=identifier_id,
...
def identifier_promote_view(request: ExtendedRequest, identifier_id: str) -> JsonResponse:
    return ResponseProvider.bad_request(
        error='contact_error', message='Primary identifier promotion is no longer supported.'
    )
...
def identifier_remove_view(request: ExtendedRequest, identifier_id: str) -> JsonResponse:
    return ResponseProvider.bad_request(
        error='contact_error', message='Removing email/phone is not supported.'
    )
...
def reward_referrer_referrals_view(request: ExtendedRequest, system_user_id: str) -> JsonResponse:
    try:
        ...
        rewarded = referral_service.reward_referrals(
            referrer=referrer, referral_ids=request.data.get('referral_ids')
        )
