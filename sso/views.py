...
def _get_client(data: dict, key: str = 'client_id') -> SystemClient | None:
...
def list_contexts_view(request: ExtendedRequest) -> JsonResponse:
    try:
        ...
        contexts = [
            vars(ctx) for ctx in sso_service.get_ready_contexts(request.sso_session, client)
        ]
...
def logout_system_view(request: ExtendedRequest) -> JsonResponse:
    try:
        ...
        if not system:
            return ResponseProvider.bad_request(error='invalid_system', message='System not found.')
