...
from utils.common import dummy_bcrypt, generate_otp, hash_value, mask
...
class SSOService:
    ...
    def authenticate_social(
        ...
        try:
            ...
            raise AuthenticationError(exc.messages[0]) from exc
        ...
        try:
            social = SocialAccount.objects.select_related('user').get(
                provider=provider,
                uid=uid,
                user__realm=client.system.realm,
            )
        except SocialAccount.DoesNotExist as exc:
            raise AuthenticationError('Invalid social account.') from exc
    ...
    def initiate_passwordless(
        self, login_value: str, client: SystemClient, ip_address: str = ''
    ) -> dict:
        config = client.get_effective_config()
    ...
    def initiate_magic_link(
        self, email: str, client: SystemClient, scopes: list, ip_address: str = ''
    ) -> dict:
        config = client.get_effective_config()
    ...
    def get_login_contexts(session: SSOSession, system: System) -> list:
        system_users = (
            SystemUser.objects.filter(
                user=session.user, system=system, status=SystemUserStatus.ACTIVE
            )
    ...
    def verify_system_mfa_otp(
        ...
        method = (
            MFAMethod.SMS if challenge.contact_type == IdentifierType.PHONE else MFAMethod.EMAIL
        )
    ...
    def select_context(
        ...
        system_user = SystemUser.objects.select_related(
            'system', 'role', 'organization', 'country'
        ...
        if requirement.required:
            allowed_method_names = (
                ...
                if requirement.allowed_methods
                else {MFAMethod.SMS, MFAMethod.EMAIL}
            )
            if self._session_mfa_satisfied(
                session, allowed_method_names, requirement.reauth_window_minutes
            ):
                pass
            else:
    ...
    def satisfy_pending_context_mfa(self, session: SSOSession, pending_context_id: str) -> str:
        ...
        except PendingContextMFA.DoesNotExist as exc:
            raise AuthenticationError(
                'Pending MFA context not found or already satisfied.'
            ) from exc
    ...
    def exchange_code(
        ...
        except AuthorizationCode.DoesNotExist as exc:
            raise OAuthError(
    ...
    def refresh_tokens(self, raw_refresh_token: str, client: SystemClient) -> TokenResponse:
        ...
        try:
            ...
            raise OAuthError('invalid_grant', 'Refresh token not found.') from exc
    ...
    def logout_all(self, user: User, reason: str = 'global_logout', ip_address: str = '') -> int:
        ...
        self._audit(
            AuditEventType.LOGOUT_GLOBAL, user, ip=ip_address, payload={'sessions_revoked': count}
        )
        return count
    ...
    def _check_system_mfa(self, session: SSOSession, client: SystemClient) -> None:
        ...
        allowed_method_names = set(system.allowed_mfa_methods or [MFAMethod.SMS, MFAMethod.EMAIL])
    ...
    def _get_system_access(
        session: SSOSession | None, system: System
    ...
    def _consume_mfa_challenge(
        ...
        except PasswordlessChallenge.DoesNotExist as exc:
            raise AuthenticationError('Invalid or expired MFA code.') from exc
    ...
    def _rate_limit_passwordless(login_value: str, ip_address: str) -> None:
        ...
        if (
            PasswordlessChallenge.objects.filter(
        ...
        if (
            ip_address
            and PasswordlessChallenge.objects.filter(
    ...
    def _rate_limit_otp(destination: str, ip_address: str) -> None:
        ...
        if (
            ...
            raise AuthenticationError('Too many MFA codes requested. Please wait a few minutes.')
        ...
        if (
            ...
            raise AuthenticationError('Too many requests from this IP. Try again later.')
