...
def system_list_view(request: ExtendedRequest) -> JsonResponse:
    try:
        ...
        return ResponseProvider.success(systems=[_system_payload(system) for system in qs])
...
def client_list_view(request: ExtendedRequest, system_id: str) -> JsonResponse:
    try:
        ...
        return ResponseProvider.success(clients=[_client_payload(client) for client in qs])
    except Exception as e:
...
def setting_list_view(request: ExtendedRequest, system_id: str) -> JsonResponse:
    try:
        ...
        return ResponseProvider.success(
            settings=[
                _setting_payload(setting) for setting in system.settings.all().order_by('key')
            ]
        )
    except Exception as e:
